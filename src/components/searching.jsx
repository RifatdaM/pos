    ...
    <div className="card p-3 md:p-4">
      <div className="flex flex-col md:flex-row gap-3">
        ...
        <div className="md:w-48 lg:w-56">
          <label htmlFor="branch-select" className="block text-xs font-medium text-gray-700 mb-1">
            Branch
          </label>
          <select
            id="branch-select"
        ...
        <div className="flex-1">
          <label htmlFor="barcode-input" className="block text-xs font-medium text-gray-700 mb-1">
            <span className="flex items-center gap-1.5">
              ...
              Scan / Search
              <kbd className="hidden md:inline px-1.5 py-0.5 text-[10px] text-gray-500 bg-gray-100 rounded">F2</kbd>
            </span>
          </label>
          <form onSubmit={handleBarcodeSubmit} className="relative">
            <input
              ...
              className={`input pl-9 pr-16 text-sm py-2 ${isScanning ? "scan-active border-blue-500" : ""}`}
              autoComplete="off"
            />
            <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <svg className={`w-4 h-4 ${isScanning ? "text-blue-600" : "text-gray-400"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </div>
            <div className="absolute inset-y-0 right-0 flex items-center pr-1.5">
              <button type="submit" className="btn btn-primary h-7 px-2.5 text-xs" disabled={!barcode.trim()}>
